...
const SERVICE_ALIASES: Record<string, string> = {
  ...
  branding: "branding", "identite-visuelle": "branding",
  maps: "maps", "google-maps": "maps",
  social: "social", "community-management": "social", "social-media": "social",
  maintenance: "maintenance", webcare: "maintenance", "maintenance-cloud": "maintenance",
  ecommerce: "ecommerce", "e-commerce": "ecommerce", "boutique-en-ligne": "ecommerce",
  seo: "seo", "referencement-naturel": "seo",
};

export const Route = createFileRoute("/services/$serviceId")({
  loader: ({ params }) => {
    const raw = params.serviceId.toLowerCase();
    const service = SERVICES.find((item) => item.id === (SERVICE_ALIASES[raw] ?? raw));
...
function ServiceDetailPage() {
  ...
  const waMessage = encodeURIComponent(`Bonjour XR Agency, je souhaite parler du service « ${service.title[lang]} »${plan ? ` et du forfait « ${plan.name[lang]} »` : ""}.`);
      ...
      <main className="relative z-10 pt-28">
        <div className="mx-auto max-w-7xl px-6 pt-4 lg:px-10">
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border/60 pb-4">
            <Link to="/services" className="label-mono inline-flex items-center gap-2 text-muted-foreground transition-colors hover:text-primary"><ArrowLeft className="h-4 w-4" />{t({ fr: "Catalogue des services", en: "Services catalogue", vi: "Danh mục dịch vụ" })}</Link>
            <div className="label-mono text-xs text-muted-foreground"><Link to="/" className="hover:text-primary">Accueil</Link> / <Link to="/services" className="hover:text-primary">Services</Link> / {t(service.title)}</div>
          </div>
        </div>
        ...
        <section className="py-16 lg:py-24">
          <div className="mx-auto grid max-w-7xl gap-12 px-6 lg:grid-cols-[1.05fr_.95fr] lg:items-end lg:px-10">
            <div>
              <Reveal><div className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/5 px-4 py-1.5 text-primary"><Sparkles className="h-3.5 w-3.5" /><span className="label-mono text-xs uppercase tracking-widest">{service.num} / 07</span></div></Reveal>
              <Reveal delay={80}><h1 className="display-serif mt-6 text-[clamp(2.8rem,7vw,6.4rem)] leading-[.95] tracking-tight">{t(service.title)}</h1></Reveal>
              <Reveal delay={160}><p className="mt-6 max-w-3xl text-lg leading-relaxed text-muted-foreground sm:text-xl">{t(service.description)}</p></Reveal>
              <Reveal delay={240}><div className="mt-9 flex flex-wrap gap-3"><a href="#plans" className="inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3.5 text-xs font-semibold uppercase tracking-widest text-primary-foreground">{t({ fr: "Voir les offres", en: "View offers", vi: "Xem các gói" })}<ArrowRight className="h-4 w-4" /></a><a href={waUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-border bg-card/70 px-6 py-3.5 text-xs font-semibold uppercase tracking-widest hover:border-primary hover:text-primary"><MessageCircle className="h-4 w-4" />WhatsApp</a></div></Reveal>
            </div>
            {image && <Reveal delay={120} className="overflow-hidden rounded-[2rem] border border-border/70 bg-card"><img src={image} alt={t(service.title)} className="aspect-[4/3] w-full object-cover" loading="eager" /></Reveal>}
          </div>
        </section>
        ...
        <section id="plans" className="py-20 lg:py-28">
          <div className="mx-auto max-w-7xl px-6 lg:px-10">
            <Reveal><p className="label-mono text-xs uppercase tracking-widest text-primary">{t({ fr: "Les offres", en: "Offers", vi: "Các gói dịch vụ" })}</p><h2 className="display-serif mt-4 max-w-4xl text-4xl sm:text-6xl">{t({ fr: "Choisissez le niveau d'accompagnement adapté à votre projet.", en: "Choose the level of support that fits your project.", vi: "Chọn cấp độ đồng hành phù hợp với dự án của bạn." })}</h2></Reveal>
            <div className="mt-12 grid gap-6 lg:grid-cols-3">{service.plans.map((item, index) => { const active = index === selectedPlan; return <button key={index} type="button" onClick={() => setSelectedPlan(index)} className={`rounded-[2rem] border p-7 text-left transition-all duration-300 sm:p-8 ${active ? "border-primary bg-primary/[0.06] shadow-xl" : "border-border bg-card/70 hover:border-primary/40"}`}><div className="flex items-start justify-between gap-4"><span className="label-mono text-xs text-primary">0{index + 1} / {String(service.plans.length).padStart(2, "0")}</span>{item.popular && <span className="label-mono rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-[9px] text-primary">{t({ fr: "RECOMMANDÉ", en: "RECOMMENDED", vi: "ĐỀ XUẤT" })}</span>}</div><h3 className="display-serif mt-5 text-3xl">{t(item.name)}</h3>{item.audience && <p className="mt-3 text-sm text-muted-foreground">{t(item.audience)}</p>}<div className="mt-7 flex items-end gap-2 border-y border-border/60 py-5"><span className="display-serif text-4xl text-primary">{formatPrice(item.eur, lang)}</span><span className="label-mono mb-1 text-xs text-muted-foreground">{t(PERIOD_LABEL[item.period])}</span></div><ul className="mt-6 space-y-3">{item.features.slice(0, 8).map((feature, featureIndex) => <li key={featureIndex} className="flex gap-3 text-sm leading-relaxed"><Check className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><span>{t(feature)}</span></li>)}</ul></button>; })}</div>
            {plan && <div className="mt-8 flex flex-col gap-5 rounded-[2rem] border border-primary/30 bg-primary/[0.05] p-7 sm:flex-row sm:items-center sm:justify-between sm:p-9"><div><p className="label-mono text-xs uppercase tracking-widest text-primary">{t({ fr: "Sélection", en: "Selected", vi: "Đã chọn" })}</p><p className="mt-2 text-lg font-medium">{t(plan.name)} · {formatPrice(plan.eur, lang)}</p></div><a href={waUrl} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-6 py-3.5 text-xs font-semibold uppercase tracking-widest text-primary-foreground">{t({ fr: "Démarrer mon projet", en: "Start my project", vi: "Bắt đầu dự án" })}<ArrowRight className="h-4 w-4" /></a></div>}
          </div>
        </section>
