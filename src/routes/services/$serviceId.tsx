...
function ServiceDetail(){
  ...
  return <div className="min-h-screen overflow-x-hidden bg-background text-foreground" style={{"--service-tint":tint} as React.CSSProperties}>
    ...
    {scrolled&&<div className="fixed inset-x-0 bottom-0 z-50 border-t border-border/70 bg-background/90 p-3 backdrop-blur-xl sm:hidden"><a href="/#quote" className="flex min-h-12 items-center justify-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground">{copy.cta}<ArrowRight className="h-4 w-4"/></a></div>}
    <Contact/>
  </div>;
}
