    ...
    >
      ...
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 lg:px-10">
            ...
            if (item.hasDropdown) {
              return (
                ...
                >
                  ...
                  {/* Mega Dropdown preview for services */}
                  {servicesDropdownOpen && (
                    <div className="absolute left-1/2 top-full -translate-x-1/2 pt-2 w-[480px]">
                      <div className="overflow-hidden rounded-2xl border border-border bg-card/98 p-4 shadow-2xl backdrop-blur-2xl animate-in fade-in zoom-in-95 duration-200">
                        <div className="flex items-center justify-between border-b border-border/60 pb-2.5 px-2">
                          ...
                          <Link
                            ...
                            className="label-mono inline-flex items-center gap-1 text-[11px] text-primary hover:underline font-semibold"
                          >
                            Catalogue complet
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-1.5">
                          {SERVICES.slice(0, 8).map((s) => (
                            <Link
                              ...
                              onClick={() => setServicesDropdownOpen(false)}
                              className="group flex flex-col rounded-xl p-2.5 transition-colors hover:bg-accent/50"
                            >
                              <div className="flex items-center justify-between">
                                <span className="display-serif text-sm font-medium text-foreground group-hover:text-primary transition-colors">
                                  {t(s.title)}
                                </span>
                                <span className="label-mono text-[9px] text-primary/80">
                                  {s.num}
                                </span>
                              </div>
                        ...
                        <div className="mt-2.5 rounded-xl border border-border/60 bg-accent/20 p-2.5 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <Sparkles className="h-3.5 w-3.5 text-primary" />
                            <span className="label-mono text-xs text-foreground font-medium">
                              SEO Domination System & Google Maps
                            </span>
                          </div>
                          <Link
                            to="/services/$serviceId"
                            ...
                            onClick={() => setServicesDropdownOpen(false)}
                            className="label-mono text-[10px] font-semibold text-primary underline"
                          >
                            Voir SEO Domination →
                          </Link>
                        </div>
            ...
            if (isHome) {
              return (
                <a
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    handleNavClick(item.section, item.href);
                  }}
                  className={cn(
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="hidden items-center gap-1 rounded-full border border-border bg-card/60 px-1 py-1 sm:flex">
            {LANGS.map((l) => (
              <button
                ...
                className={cn(
                  "label-mono rounded-full px-2.5 py-0.5 text-xs transition-colors cursor-pointer",
                  lang === l.code
          ...
          {/* Book Call CTA */}
          <EmberButton href="/#contact" className="hidden md:inline-flex text-xs">
            {t(UI.bookCall)}
          </EmberButton>
          ...
          <button
            aria-label="Menu"
            onClick={() => setOpen((v) => !v)}
            className="flex h-10 w-10 items-center justify-center rounded-full border border-border bg-card/80 lg:hidden cursor-pointer"
          >
            {open ? (
        ...
        <div className="border-t border-border bg-background/98 px-6 py-6 backdrop-blur-2xl lg:hidden animate-in slide-in-from-top-2 duration-200">
          <div className="flex flex-col gap-1.5">
            <Link
              ...
              className="label-mono flex items-center justify-between rounded-xl px-4 py-3 text-sm text-foreground hover:bg-accent/40"
            >
            ...
            {/* Quick sub-links for main services */}
            ...
            {NAV_ITEMS.filter((i) => !i.hasDropdown).map((item) => (
              <a
                ...
                className="label-mono rounded-xl px-4 py-3 text-sm text-muted-foreground transition-colors hover:bg-accent/40 hover:text-foreground"
              >
                {t(UI[item.labelKey])}
              </a>
            ))}
            ...
            <a
              href="/#contact"
              onClick={() => setOpen(false)}
              className="label-mono mt-3 rounded-full bg-primary px-6 py-3 text-center text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90"
            >
              {t(UI.bookCall)}
            </a>
              ...
              <div className="flex gap-1.5">
                {LANGS.map((l) => (
                  <button
                    ...
                    onClick={() => setLang(l.code)}
                    className={cn(
                      "label-mono rounded-full border px-2.5 py-1 text-xs transition-colors",
                      lang === l.code
