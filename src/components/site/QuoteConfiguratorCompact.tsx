...
export function QuoteConfiguratorCompact() {
  ...
  const recommendedIds = useMemo(() => profileRecommendation, [profileRecommendation]);
  ...
  return <section id="quote" className="relative border-y border-border/50 bg-background py-4 sm:py-6"><div className="mx-auto max-w-2xl px-3.5 sm:px-5"><div className="mb-2 flex items-center justify-between gap-3"><div className="flex items-center gap-2"><div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-primary/30 bg-primary/[0.08] text-primary shadow-sm"><span className="absolute -right-1 -top-1 h-2.5 w-2.5 rounded-full bg-primary" /><span className="text-sm font-bold">A</span></div><div><div className="label-mono text-[8px] uppercase tracking-[0.2em] text-primary">XR INTELLIGENCE</div><div className="text-[10px] text-muted-foreground sm:text-xs">Alexandre vous accompagne · devis personnalisé</div></div></div><div className="flex items-center gap-1.5"><div className="h-1 w-10 overflow-hidden rounded-full bg-muted sm:w-16"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${progress}%` }} /></div><span className="label-mono text-[7px] text-muted-foreground">{String(step + 1).padStart(2, "0")}/08</span></div></div>
    <div className="rounded-2xl border border-border bg-card/80 p-3 shadow-[0_20px_60px_-45px_rgba(0,0,0,.6)] sm:p-5"><div className="mb-3 flex items-end justify-between gap-2"><div><p className="label-mono text-[7px] uppercase tracking-[0.18em] text-muted-foreground">{step >= 7 ? "Finalisation" : `Question ${step + 1} sur 8`}</p><h2 className="mt-1 text-base font-medium tracking-[-0.03em] sm:text-2xl">{titles[step]}</h2></div>{step > 0 && step < 7 && <button type="button" onClick={() => setStep(step - 1)} className="rounded-full border border-border px-2.5 py-1 text-[10px] text-muted-foreground hover:text-foreground">←</button>}</div>
      ...
      {step === 3 && <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">{BUDGETS.map((x) => card(x, () => { setBudget(x.id); const base = profileRecommendation; setSelectedServices(base); window.setTimeout(() => setStep(4), 180); }, x.id === budget))}</div>}
      {step === 4 && <div><div className="mb-3 rounded-xl border border-primary/25 bg-primary/[0.055] p-3"><div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wider text-primary"><Sparkles className="h-3.5 w-3.5" /> Sélection recommandée par Alexandre</div><p className="mt-1 text-[10px] leading-4 text-muted-foreground">La sélection tient compte de votre secteur, objectif, situation, budget et canal d'acquisition. Elle n'est pas automatiquement poussée vers l'offre la plus chère.</p></div><div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">{orderedServices.map((x) => card(x, () => toggleService(x.id), selectedServices.includes(x.id) || activeRecommended.includes(x.id)))}</div><button type="button" disabled={!selectedServices.length} onClick={() => setStep(5)} className="mt-3 w-full rounded-full bg-primary px-5 py-3 text-[10px] font-semibold uppercase tracking-wider text-primary-foreground disabled:opacity-40">Valider ma sélection →</button></div>}
