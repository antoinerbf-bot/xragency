...
const SECTORS: Sector[] = [
  ...
  { id: "other", label: "Autre activité", detail: "Votre activité ne figure pas ici ? Julie s'adapte.", goals: ["Développer mon activité", "Professionnaliser mon image", "Construire une présence forte"] },
];
...
export function QuoteConfiguratorCompact() {
  ...
  const recommendation = useMemo(() => {
    ...
    return `Pour ${sector.label.toLowerCase()}, Julie recommande ${budgetText}, en donnant la priorité à ${activeRecommended.map((id) => SERVICES.find((x) => x.id === id)?.label).filter(Boolean).join(", ")}. Votre choix de ${channel} renforce cette recommandation.`;
  }, [sector, discoveryChoice, discovery, budget, activeRecommended]);

  const titles = ["Votre activité", "Votre priorité", "Votre situation aujourd'hui", "L'investissement envisagé", "Les prestations à activer", "Comment vos clients vous trouvent", "Votre recommandation", "Vos informations"]; const progress = ((step + 1) / 8) * 100; const card = (item: Choice, onClick: () => void, selected = false) => <button key={item.id} type="button" onClick={onClick} className={`group relative w-full overflow-hidden rounded-[1.15rem] border p-3.5 text-left transition-all duration-300 hover:-translate-y-1 hover:border-primary/55 hover:shadow-[0_22px_45px_-25px_rgba(0,0,0,.85)] ${selected ? "border-primary bg-primary/[0.09] shadow-[0_18px_45px_-25px_hsl(var(--primary)/.45)]" : "border-border/80 bg-background/70"}`}><span className="pointer-events-none absolute -right-5 -top-5 h-16 w-16 rounded-full bg-primary/[0.05] blur-xl transition-transform duration-500 group-hover:scale-150" /><div className="relative flex items-start justify-between gap-3"><div><span className="mb-2 inline-flex rounded-full border border-border/70 bg-background/70 px-2 py-1 label-mono text-[7px] uppercase tracking-[.14em] text-muted-foreground">Julie · {selected ? "retenu" : "option"}</span><span className="block text-[11px] font-medium leading-tight sm:text-sm">{item.label}</span><span className="mt-1.5 block text-[9px] leading-4 text-muted-foreground">{item.detail}</span></div><span className={`mt-1 grid h-6 w-6 shrink-0 place-items-center rounded-full border transition-all ${selected ? "border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground group-hover:border-primary/50 group-hover:text-primary"}`}>{selected ? <Check className="h-3 w-3" /> : <ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-0.5" />}</span></div></button>;

  return <section id="quote" className="relative border-y border-border/50 bg-background py-4 sm:py-6"><div className="mx-auto max-w-2xl px-3.5 sm:px-5"><div className="mb-3 flex items-center justify-between gap-2"><div className="flex min-w-0 items-center gap-2.5"><div className="shrink-0"><JulieAdvisor mood={step >= 7 ? "success" : step >= 6 ? "recommend" : step >= 4 ? "focus" : "calm"} sector={sector?.label} goal={goal} selectedServices={selectedServices} step={step} /></div><div><div className="flex items-center gap-2 label-mono text-[8px] uppercase tracking-[0.2em] text-primary"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" /> XR INTELLIGENCE · JULIE</div><div className="truncate text-[9px] text-muted-foreground sm:text-xs">Je construis votre recommandation étape par étape.</div></div></div><div className="flex items-center gap-1.5"><div className="h-1 w-10 overflow-hidden rounded-full bg-muted sm:w-16"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${progress}%` }} /></div><span className="label-mono text-[7px] text-muted-foreground">{String(step + 1).padStart(2, "0")}/08</span></div></div>
    <div className="relative overflow-hidden rounded-[1.5rem] border border-border/80 bg-card/70 p-3 shadow-[0_30px_80px_-50px_rgba(0,0,0,.9)] backdrop-blur-2xl sm:p-5"><div className="pointer-events-none absolute -right-20 -top-20 h-48 w-48 rounded-full bg-primary/10 blur-3xl" /><div className="mb-4 flex items-end justify-between gap-2 border-b border-border/70 pb-3"><div><p className="label-mono text-[7px] uppercase tracking-[0.18em] text-muted-foreground">{step >= 7 ? "Finalisation" : `Question ${step + 1} sur 8`}</p><h2 className="mt-1 text-base font-medium tracking-[-0.03em] sm:text-2xl">{titles[step]}</h2></div>{step > 0 && step < 7 && <button type="button" onClick={() => setStep(step - 1)} className="rounded-full border border-border px-2.5 py-1 text-[10px] text-muted-foreground hover:text-foreground">←</button>}</div>
      {step === 0 && <div>
        <div className="relative mb-5 overflow-hidden rounded-[2rem] border border-primary/20 bg-[linear-gradient(135deg,rgba(255,92,180,.10),rgba(44,211,255,.07),rgba(255,204,92,.08))] p-5 sm:p-6">
          <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-fuchsia-400/20 blur-3xl" />
          <div className="absolute -bottom-10 left-1/3 h-28 w-28 rounded-full bg-cyan-400/15 blur-3xl" />
          <div className="relative">
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.14em] text-primary"><Globe2 className="h-4 w-4" /> Votre activité · votre univers</div>
            <h3 className="mt-2 text-xl font-semibold tracking-tight sm:text-2xl">Quel projet voulez-vous faire grandir ?</h3>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Choisissez votre univers. Julie adaptera ensuite les questions, les priorités et les prestations à votre projet.</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
          {(showAllSectors ? SECTORS : SECTORS.slice(0,6)).map((x,i) => {
            const accents = ["from-fuchsia-500/20 to-rose-400/5","from-cyan-400/20 to-blue-400/5","from-amber-300/25 to-orange-400/5","from-violet-400/20 to-indigo-400/5","from-emerald-400/20 to-teal-400/5","from-sky-400/20 to-cyan-400/5"];
            return <button key={x.id} type="button" onClick={() => { setSectorId(x.id); window.setTimeout(() => setStep(1), 180); }} className={`group relative min-h-[118px] overflow-hidden rounded-[1.4rem] border border-border/80 bg-gradient-to-br ${accents[i % accents.length]} p-4 text-left transition-all duration-300 hover:-translate-y-1 hover:border-primary/55 hover:shadow-[0_22px_50px_-28px_hsl(var(--primary)/.7)]`}>
              <span className="absolute right-3 top-3 text-[9px] font-mono text-foreground/35">0{String(i+1)}</span>
              <span className="block pr-7 text-[12px] font-bold leading-5 transition-colors group-hover:text-primary sm:text-[14px]">{x.label}</span>
              <span className="mt-2 block text-[10px] leading-4 text-muted-foreground line-clamp-2">{x.detail}</span>
              <span className="absolute bottom-3 right-4 grid h-7 w-7 place-items-center rounded-full border border-white/10 bg-background/40 text-primary transition-all group-hover:scale-110"><ArrowRight className="h-3.5 w-3.5" /></span>
            </button>;
          })}
        </div></div><button type="button" onClick={() => setShowAllSectors(v=>!v)} className="mt-2 w-full rounded-xl border border-dashed border-border py-2.5 text-[9px] uppercase tracking-[.14em] text-muted-foreground transition hover:border-primary/35 hover:text-primary">{showAllSectors ? "Réduire les activités" : "Voir toutes les activités · Julie affine ensuite avec vous"}</button></div>}
      ...
      {step === 4 && <div><div className="mb-3 rounded-xl border border-primary/15 bg-primary/[0.035] p-3"><div className="text-[10px] font-medium text-primary">Julie a déjà fait le premier tri</div><p className="mt-1 text-[9px] leading-4 text-muted-foreground">Voici les leviers qui ont du sens pour votre activité. Ajustez-les librement avant de valider.</p></div><div className="relative mb-3 overflow-hidden rounded-[1.25rem] border border-primary/20 bg-gradient-to-br from-primary/[0.10] via-primary/[0.025] to-transparent p-3 shadow-sm sm:p-4"><div className="pointer-events-none absolute -right-8 -top-8 h-24 w-24 rounded-full bg-primary/15 blur-2xl" /><div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wider text-primary"><Sparkles className="h-3.5 w-3.5" /> Sélection préparée par Julie <span className="ml-1 rounded-full border border-primary/20 px-1.5 py-0.5 text-[8px]">profil + budget + objectif</span></div><p className="mt-2 text-[10px] leading-4 text-muted-foreground">Julie croise votre activité, votre objectif, votre situation, votre budget et votre acquisition pour éviter les prestations inutiles.</p></div><div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">{orderedServices.map((x) => card(x, () => toggleService(x.id), selectedServices.includes(x.id)))}</div><button type="button" disabled={!selectedServices.length} onClick={() => setStep(5)} className="mt-3 w-full rounded-full bg-primary px-5 py-3 text-[10px] font-semibold uppercase tracking-wider text-primary-foreground disabled:opacity-40">Valider ma sélection →</button></div>}
