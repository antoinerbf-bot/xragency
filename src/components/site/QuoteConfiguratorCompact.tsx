...
export function QuoteConfiguratorCompact() {
  ...
  return <section id="quote" className="relative border-y border-border/50 bg-background py-4 sm:py-6"><div className="mx-auto max-w-2xl px-3.5 sm:px-5"><div className="mb-3 flex items-center justify-between gap-2"><div className="flex min-w-0 items-center gap-2.5"><div className="shrink-0"><UniversePresence mood={step >= 7 ? "success" : step >= 6 ? "recommend" : step >= 4 ? "focus" : "calm"} sector={sector?.id} step={step} /></div><div><div className="flex items-center gap-2 label-mono text-[12px] uppercase tracking-[0.2em] text-primary"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" /> XR INTELLIGENCE</div><div className="truncate text-[14px] text-muted-foreground sm:text-xs">Je construis votre recommandation étape par étape.</div></div></div><div className="flex items-center gap-1.5"><div className="h-1 w-10 overflow-hidden rounded-full bg-muted sm:w-16"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${progress}%` }} /></div><span className="label-mono text-[12px] text-muted-foreground">{String(step + 1).padStart(2, "0")}/08</span></div></div>
