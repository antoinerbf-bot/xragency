...
export function QuoteConfiguratorCompact() {
  ...
  const titles = ["Votre activité", "Votre priorité", "Votre situation aujourd'hui", "L'investissement envisagé", "Les prestations à activer", "Comment vos clients vous trouvent", "Votre recommandation", "Vos informations"]; const progress = ((step + 1) / 8) * 100; const card = (item: Choice, onClick: () => void, selected = false) => <button key={item.id} type="button" onClick={onClick} className={`group relative w-full overflow-hidden rounded-[1.15rem] border p-3.5 text-left transition-all duration-300 hover:-translate-y-1 hover:border-primary/55 hover:shadow-[0_22px_45px_-25px_rgba(0,0,0,.85)] ${selected ? "border-primary bg-primary/[0.09] shadow-[0_18px_45px_-25px_hsl(var(--primary)/.45)]" : "border-border/80 bg-background/70"}`}><span className="pointer-events-none absolute -right-5 -top-5 h-16 w-16 rounded-full bg-primary/[0.05] blur-xl transition-transform duration-500 group-hover:scale-150" /><div className="relative flex items-start justify-between gap-3"><div><span className="mb-2 inline-flex rounded-full border border-border/70 bg-background/70 px-2 py-1 label-mono text-xs uppercase tracking-[.14em] text-muted-foreground">XR Intelligence · {selected ? "retenu" : "option"}</span><span className="block text-[15px] font-medium leading-tight sm:text-base">{item.label}</span><span className="mt-1.5 block text-[13px] leading-5 text-muted-foreground">{item.detail}</span></div><span className={`mt-1 grid h-6 w-6 shrink-0 place-items-center rounded-full border transition-all ${selected ? "border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground group-hover:border-primary/50 group-hover:text-primary"}`}>{selected ? <Check className="h-3 w-3" /> : <ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-0.5" />}</span></div></button>;
