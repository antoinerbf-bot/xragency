...
const SECTORS: Sector[] = [
  ...
  { id: "local", label: "Entreprise locale", detail: "Service, commerce, activité de proximité", goals: ["Être trouvé localement", "Recevoir plus de demandes", "Professionnaliser l'image"] },
  { id: "other", label: "Autre activité", detail: "Alexandre s'adapte à votre métier", goals: ["Développer mon activité", "Professionnaliser mon image", "Construire une présence forte"] },
];
...
export function QuoteConfiguratorCompact() {
  ...
  const recommendation = useMemo(() => {
    ...
    const priority = goalText.includes("réservation") || goalText.includes("rendez-vous")
      ? "La priorité sera le parcours de réservation et la conversion."
      : goalText.includes("google") || goalText.includes("local") || goalText.includes("visible")
        ? "La priorité sera la visibilité sur les points de recherche les plus rentables."
        : goalText.includes("vente") || goalText.includes("vendre")
          ? "La priorité sera le parcours commercial et la conversion."
          : goalText.includes("premium") || goalText.includes("gamme")
            ? "La priorité sera la perception de marque et l'expérience proposée."
            : "La stratégie sera calibrée selon votre situation, votre budget et votre objectif.";
  ...
  const titles = [
    "D'abord, votre activité.",
    "Votre priorité ?",
    "Votre situation aujourd'hui ?",
    "Quelle expertise activer ?",
  ...
  const card = (item: Choice, onClick: () => void, compact = false) => (
    <button key={item.id} type="button" onClick={onClick} className={`group rounded-lg border border-border bg-background text-left transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/60 hover:shadow-md active:scale-[.99] ${compact ? "px-2.5 py-2" : "p-3"}`}>
  ...
  return (
    <section id="quote" className="relative border-y border-border/50 bg-background py-4 sm:py-6">
      <div className="mx-auto max-w-2xl px-3.5 sm:px-6">
        <div className="mb-2 flex items-center justify-between gap-2">
          ...
          <div className="flex shrink-0 items-center gap-1.5"><div className="h-1 w-10 overflow-hidden rounded-full bg-muted sm:w-14"><div className="h-full rounded-full bg-primary transition-all duration-500" style={{ width: `${progress}%` }} /></div><span className="label-mono text-[7px] text-muted-foreground">{String(step + 1).padStart(2, "0")}/07</span></div>
        </div>

        <div className="rounded-xl border border-border bg-card/70 p-3 shadow-[0_14px_45px_-35px_rgba(0,0,0,.5)] sm:p-4">
          <div className="mb-2.5 flex items-end justify-between gap-2"><div><p className="label-mono text-[7px] uppercase tracking-[0.18em] text-muted-foreground">{step === 6 ? "Analyse terminée" : `Question ${step + 1} sur 7`}</p><h2 className="mt-0.5 text-base font-medium tracking-[-0.03em] sm:text-xl">{titles[step]}</h2></div>{step > 0 && <button type="button" onClick={() => setStep(step - 1)} className="rounded-full border border-border px-2 py-1 text-[10px] text-muted-foreground hover:text-foreground" aria-label="Question précédente">←</button>}</div>

          <div key={step} className="animate-in fade-in slide-in-from-right-1 duration-200">
            {step === 0 && <div><p className="mb-1.5 text-[9px] text-muted-foreground">Choisissez simplement votre univers — Alexandre adaptera la suite du diagnostic.</p><div className="flex gap-1.5 overflow-x-auto pb-1 snap-x snap-mandatory" style={{ scrollbarWidth: "none" }}>{SECTORS.map((x) => <div key={x.id} className="min-w-[142px] snap-start">{card(x, () => choose(setSectorId, x.id, 1), true)}</div>)}</div></div>}
            {step === 1 && sector && <div><p className="mb-1.5 text-[9px] text-muted-foreground">Une seule réponse suffit. Cela permet de personnaliser les prestations proposées.</p><div className="grid grid-cols-1 gap-1.5 sm:grid-cols-3">{sector.goals.map((x) => card({ id: x, label: x, detail: "Priorité adaptée à votre secteur" }, () => choose(setGoal, x, 2)))}</div></div>}
            ...
            {step === 3 && <div><p className="mb-1.5 text-[9px] text-muted-foreground">Nous activons ensuite l'expertise la plus pertinente pour votre objectif.</p><div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">{SERVICES.map((x) => card(x, () => choose(setServiceId, x.id, 4)))}</div></div>}
            {step === 4 && <div><p className="mb-1.5 text-[9px] text-muted-foreground">Votre budget nous aide à proposer une recommandation réaliste, pas un catalogue.</p><div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-5">{BUDGETS.map((x) => card(x, () => choose(setBudget, x.id, 5)))}</div></div>}
            {step === 5 && <div><p className="mb-1.5 text-[9px] text-muted-foreground">Dernière question : cela nous indique où renforcer votre acquisition.</p><div className="grid grid-cols-2 gap-1.5 sm:grid-cols-4">{DISCOVERY.map((x) => card(x, () => choose(setDiscovery, x.id, 6))}</div></div>}
            {step === 6 && sector && service && situationChoice && budgetChoice && discoveryChoice && <div className="space-y-2.5">
              <div className="rounded-lg border border-primary/30 bg-primary/[0.06] p-3"><div className="flex items-center gap-1.5 text-[11px] font-medium"><Check className="h-3.5 w-3.5 text-primary" /> Recommandation XR Intelligence</div><p className="mt-1.5 text-xs leading-5 sm:text-sm">{recommendation}</p></div>
              <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-4">{[["Secteur", sector.label], ["Objectif", goal], ["Situation", situationChoice.label], ["Budget", budgetChoice.label]].map(([label, value]) => <div key={label} className="rounded-lg border border-border bg-background p-2"><div className="label-mono text-[6px] uppercase tracking-wider text-muted-foreground">{label}</div><div className="mt-0.5 text-[9px] leading-4">{value}</div></div>)}</div>
              <div className="flex flex-wrap gap-1.5"><a href={`${CONTACT.whatsapp}?text=${encodeURIComponent(`Bonjour XR Agency, je souhaite un devis sur mesure. Secteur : ${sector.label}. Objectif : ${goal}. Situation : ${situationChoice.label}. Expertise : ${service.label}. Budget : ${budgetChoice.label}. Acquisition : ${discoveryChoice.label}.`)}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-full bg-primary px-3.5 py-2 text-[8px] font-semibold uppercase tracking-wider text-primary-foreground">Recevoir mon devis <ArrowRight className="h-3 w-3" /></a><button type="button" onClick={reset} className="inline-flex items-center gap-1.5 rounded-full border border-border px-3.5 py-2 text-[8px] font-semibold uppercase tracking-wider text-muted-foreground hover:text-foreground"><RotateCcw className="h-3 w-3" /> Recommencer</button></div>
            </div>}
          </div>
        </div>
      </div>
