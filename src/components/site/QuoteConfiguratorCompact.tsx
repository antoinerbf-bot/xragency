...
export function QuoteConfiguratorCompact() {
  ...
  const recommendation = useMemo(() => {
    ...
    const serviceText: Record<string, string> = {
      website: situation === "none" ? "Une création web pensée pour votre activité et votre objectif." : "Une refonte ou évolution web orientée conversion et expérience utilisateur.",
    ...
    return `${serviceText[service.id]} ${priority}`;
  ...
  const card = (item: Choice, onClick: () => void, compact = false) => (
    <button key={item.id} type="button" onClick={onClick} className={`group rounded-xl border border-border bg-background text-left transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/60 hover:shadow-md active:scale-[.99] ${compact ? "px-3 py-2.5" : "p-3"}`}>
      <div className="flex items-center justify-between gap-2"><span className="text-[12px] font-medium leading-tight sm:text-sm">{item.label}</span><ArrowRight className="h-3.5 w-3.5 shrink-0 text-muted-foreground transition-transform group-hover:translate-x-1 group-hover:text-primary" /></div>
      <div className="mt-1 hidden text-[10px] leading-4 text-muted-foreground sm:block">{item.detail}</div>
    </button>
  );

  return (
    <section id="quote" className="relative border-y border-border/50 bg-background py-5 sm:py-7">
      <div className="mx-auto max-w-3xl px-4 sm:px-6">
        <div className="mb-2.5 flex items-center justify-between gap-3">
          ...
          <div className="flex shrink-0 items-center gap-1.5"><div className="h-1 w-12 overflow-hidden rounded-full bg-muted sm:w-16"><div className="h-full rounded-full bg-primary transition-all duration-500" style={{ width: `${progress}%` }} /></div><span className="label-mono text-[8px] text-muted-foreground">{String(step + 1).padStart(2, "0")}/07</span></div>
        </div>

        <div className="rounded-2xl border border-border bg-card/80 p-3.5 shadow-[0_18px_60px_-42px_rgba(0,0,0,.55)] sm:p-5">
          <div className="mb-3 flex items-end justify-between gap-3"><div><p className="label-mono text-[8px] uppercase tracking-[0.18em] text-muted-foreground">{step === 6 ? "Analyse terminée" : `Question ${step + 1}`}</p><h2 className="mt-1 text-lg font-medium tracking-[-0.03em] sm:text-2xl">{titles[step]}</h2></div>{step > 0 && <button type="button" onClick={() => setStep(step - 1)} className="rounded-full border border-border p-1.5 text-xs text-muted-foreground hover:text-foreground" aria-label="Question précédente">←</button>}</div>

          <div key={step} className="animate-in fade-in slide-in-from-right-2 duration-250">
            {step === 0 && <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">{SECTORS.map((x) => card(x, () => choose(setSectorId, x.id, 1), true))}</div>}
            {step === 1 && sector && <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-3">{sector.goals.map((x) => card({ id: x, label: x, detail: "Priorité adaptée à votre secteur" }, () => choose(setGoal, x, 2)))}</div>}
            {step === 2 && <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-2">{SITUATIONS.map((x) => card(x, () => choose(setSituation, x.id, 3)))}</div>}
            {step === 3 && <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">{SERVICES.map((x) => card(x, () => choose(setServiceId, x.id, 4)))}</div>}
            {step === 4 && <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-5">{BUDGETS.map((x) => card(x, () => choose(setBudget, x.id, 5)))}</div>}
            {step === 5 && <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-4">{DISCOVERY.map((x) => card(x, () => choose(setDiscovery, x.id, 6)))}</div>}
            {step === 6 && sector && service && situationChoice && budgetChoice && discoveryChoice && <div className="space-y-2.5">
              <div className="rounded-xl border border-primary/30 bg-primary/[0.06] p-3.5"><div className="flex items-center gap-2 text-xs font-medium"><Check className="h-4 w-4 text-primary" /> Recommandation XR Intelligence</div><p className="mt-2 text-sm leading-6">{recommendation}</p></div>
