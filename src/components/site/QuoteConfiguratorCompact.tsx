...
const SECTORS: Sector[] = [
  ...
  { id: "other", label: "Autre activité", detail: "Votre métier ne figure pas ici ? Alexandre s'adapte.", goals: ["Développer mon activité", "Professionnaliser mon image", "Construire une présence forte"] },
];
...
export function QuoteConfiguratorCompact() {
  ...
  return <section id="quote" className="relative border-y border-border/50 bg-background py-4 sm:py-6"><div className="mx-auto max-w-2xl px-3.5 sm:px-5"><div className="mb-3 overflow-hidden rounded-[1.25rem] border border-primary/15 bg-gradient-to-br from-card via-card/80 to-primary/[.045] shadow-[0_18px_55px_-35px_rgba(0,0,0,.9)]"><div className="flex items-center justify-between gap-3 px-3 py-2.5 sm:px-4"><div className="flex min-w-0 items-center gap-2.5"><div className="shrink-0"><AlexandreAdvisor /></div><div className="min-w-0"><div className="flex items-center gap-1.5 text-[8px] font-semibold uppercase tracking-[.18em] text-primary"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500"/>Alexandre · conseiller digital</div><div className="mt-1 truncate text-[9px] text-muted-foreground sm:text-[11px]">Je transforme vos réponses en plan d’action.</div></div></div><div className="hidden items-center gap-2 sm:flex"><span className="rounded-full border border-border bg-background/70 px-2 py-1 text-[7px] uppercase tracking-[.14em] text-muted-foreground">sur mesure</span><span className="rounded-full border border-emerald-500/20 bg-emerald-500/5 px-2 py-1 text-[7px] text-emerald-600">● en ligne</span></div></div><div className="grid grid-cols-4 border-t border-border/60 bg-background/35">{["Activité","Objectif","Situation","Budget"].map((label,i)=><div key={label} className={"border-r border-border/50 px-2 py-2 last:border-r-0 "+(step >= i ? "text-foreground" : "text-muted-foreground/50")}><span className="label-mono block text-[6px] uppercase tracking-[.12em]">{String(i+1).padStart(2,"0")}</span><span className="mt-0.5 block truncate text-[7px] sm:text-[8px]">{label}</span></div>)}</div><div className="h-0.5 bg-muted"><div className="h-full bg-primary transition-all duration-500" style={{ width: `${progress}%` }} /></div></div>    <div className="relative overflow-hidden rounded-[1.5rem] border border-border/80 bg-card/70 p-3 shadow-[0_30px_80px_-50px_rgba(0,0,0,.9)] backdrop-blur-2xl sm:p-5"><div className="pointer-events-none absolute -right-20 -top-20 h-48 w-48 rounded-full bg-primary/10 blur-3xl" /><div className="mb-4 flex items-end justify-between gap-2 border-b border-border/70 pb-3"><div><p className="label-mono text-[7px] uppercase tracking-[0.18em] text-muted-foreground">{step >= 7 ? "Finalisation" : `Question ${step + 1} sur 8`}</p><h2 className="mt-1 text-base font-medium tracking-[-0.03em] sm:text-2xl">{titles[step]}</h2></div>{step > 0 && step < 7 && <button type="button" onClick={() => setStep(step - 1)} className="rounded-full border border-border px-2.5 py-1 text-[10px] text-muted-foreground hover:text-foreground">←</button>}</div>
      ...
      {step === 7 && <div className="space-y-2.5"><p className="text-[9px] text-muted-foreground">Entrez vos coordonnées. Alexandre prépare le PDF avec le contexte, les prestations sélectionnées et les montants.</p><input value={client.company} onChange={(e) => setClient({ ...client, company: e.target.value })} placeholder="Nom de la société *" className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary" /><input value={client.name} onChange={(e) => setClient({ ...client, name: e.target.value })} placeholder="Votre nom *" className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary" /><input value={client.email} onChange={(e) => setClient({ ...client, email: e.target.value })} type="email" placeholder="E-mail *" className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary" /><input value={client.whatsapp} onChange={(e) => setClient({ ...client, whatsapp: e.target.value })} placeholder="WhatsApp / téléphone *" className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary" /> <input value={client.website} onChange={(e) => setClient({ ...client, website: e.target.value })} placeholder="Site internet (si vous en avez un)" className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary" /><button type="button" disabled={!client.company || !client.name || !client.email || !client.whatsapp || sending} onClick={generatePdf} className="w-full rounded-full bg-primary px-5 py-3.5 text-[10px] font-semibold uppercase tracking-wider text-primary-foreground disabled:opacity-40">{sending ? "Génération et envoi…" : generated ? "Générer / renvoyer le devis PDF →" : "Générer mon devis PDF →"}</button>{sendMessage && <div className="rounded-xl border border-primary/25 bg-primary/[0.05] p-3 text-center text-[10px] leading-5 text-muted-foreground">{sendMessage}</div>}<div className="grid grid-cols-2 gap-2"><a href={`https://wa.me/33767566783?text=${encodeURIComponent(`Bonjour XR Agency, je viens de préparer un devis avec Alexandre. Nom : ${client.name}. Email : ${client.email}. WhatsApp : ${client.whatsapp}. Total estimé : ${once.toLocaleString("fr-FR")} € + ${monthly.toLocaleString("fr-FR")} €/mois.`)}`} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl border border-border px-3 py-3 text-[10px] font-semibold uppercase tracking-wider"><MessageCircle className="h-3.5 w-3.5" /> WhatsApp</a><a href={`mailto:${client.email}?subject=${encodeURIComponent("Votre devis XRAGENCY")}&body=${encodeURIComponent("Votre devis PDF a été généré avec Alexandre. Si l'envoi automatique n'est pas disponible, vous pouvez partager le PDF généré depuis votre appareil.")}`} className="inline-flex items-center justify-center gap-2 rounded-xl border border-border px-3 py-3 text-[10px] font-semibold uppercase tracking-wider"><Mail className="h-3.5 w-3.5" /> E-mail</a></div><button type="button" onClick={reset} className="mx-auto flex items-center gap-1 text-[9px] text-muted-foreground hover:text-foreground"><RotateCcw className="h-3 w-3" /> Recommencer</button></div>}
    </div></div></section>;
}
