...
export function QuoteConfiguratorCompact() {
  ...
  return <section id="quote" className="relative border-y border-border/50 bg-background py-4 sm:py-6"><div className="mx-auto max-w-2xl px-3.5 sm:px-5"><div className="mb-3 flex items-center justify-between gap-3"><div className="flex items-center gap-3"><div className="scale-[0.72] -ml-3 -my-7 sm:scale-[0.62] sm:-ml-4 sm:-my-8"><AlexandreAdvisor /></div><div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-primary/30 bg-primary/[0.08] text-primary shadow-sm"><span className="absolute -right-1 -top-1 h-2.5 w-2.5 rounded-full bg-primary" /><span className="text-sm font-bold">A</span></div><div><div className="flex items-center gap-2 label-mono text-[8px] uppercase tracking-[0.2em] text-primary"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" /> XR INTELLIGENCE · ALEXANDRE</div><div className="flex items-center gap-2 text-[10px] text-muted-foreground sm:text-xs"><span className="hidden rounded-full border border-primary/20 bg-primary/[0.06] px-2 py-0.5 text-[7px] uppercase tracking-[.15em] text-primary sm:inline">assistant interactif</span> Je construis votre recommandation étape par étape.</div></div></div><div className="flex items-center gap-1.5"><div className="h-1 w-10 overflow-hidden rounded-full bg-muted sm:w-16"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${progress}%` }} /></div><span className="label-mono text-[7px] text-muted-foreground">{String(step + 1).padStart(2, "0")}/08</span></div></div>
    <div className="relative overflow-hidden rounded-[1.75rem] border border-primary/15 bg-card/80 p-3 shadow-[0_35px_100px_-50px_rgba(0,0,0,.9)] backdrop-blur-2xl sm:p-6"><div className="pointer-events-none absolute -right-20 -top-20 h-48 w-48 rounded-full bg-primary/10 blur-3xl" /><div className="mb-4 flex items-end justify-between gap-2 border-b border-border/70 pb-3"><div><p className="label-mono text-[7px] uppercase tracking-[0.18em] text-muted-foreground">{step >= 7 ? "Finalisation" : `Question ${step + 1} sur 8`}</p><h2 className="mt-1 text-base font-medium tracking-[-0.03em] sm:text-2xl">{titles[step]}</h2></div>{step > 0 && step < 7 && <button type="button" onClick={() => setStep(step - 1)} className="rounded-full border border-border px-2.5 py-1 text-[10px] text-muted-foreground hover:text-foreground">←</button>}</div>
