...
export function QuoteConfiguratorCompact() {
  ...
  const titles = ["Votre activité", "Votre priorité", "Votre situation aujourd'hui", "L'investissement envisagé", "Les prestations à activer", "Comment vos clients vous trouvent", "Votre recommandation", "Vos informations"]; const progress = ((step + 1) / 8) * 100; const card = (item: Choice, onClick: () => void, selected = false) => <button key={item.id} type="button" onClick={onClick} className={`group relative w-full overflow-hidden rounded-[1.15rem] border p-3.5 text-left transition-all duration-300 hover:-translate-y-1 hover:border-primary/55 hover:shadow-[0_22px_45px_-25px_rgba(0,0,0,.85)] ${selected ? "border-primary bg-primary/[0.09] shadow-[0_18px_45px_-25px_hsl(var(--primary)/.45)]" : "border-border/80 bg-background/70"}`}><span className="pointer-events-none absolute -right-5 -top-5 h-16 w-16 rounded-full bg-primary/[0.05] blur-xl transition-transform duration-500 group-hover:scale-150" /><div className="relative flex items-start justify-between gap-3"><div><span className="mb-2 inline-flex rounded-full border border-border/70 bg-background/70 px-2 py-1 label-mono text-[12px] uppercase tracking-[.14em] text-muted-foreground">Julie · {selected ? "retenu" : "option"}</span><span className="block text-[15px] font-medium leading-tight sm:text-sm">{item.label}</span><span className="mt-1.5 block text-[14px] leading-4 text-muted-foreground">{item.detail}</span></div><span className={`mt-1 grid h-6 w-6 shrink-0 place-items-center rounded-full border transition-all ${selected ? "border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground group-hover:border-primary/50 group-hover:text-primary"}`}>{selected ? <Check className="h-3 w-3" /> : <ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-0.5" />}</span></div></button>;

  return <section id="quote" className="relative border-y border-border/50 bg-background py-4 sm:py-6"><div className="mx-auto max-w-2xl px-3.5 sm:px-5"><div className="mb-4 flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2.5"><div className="w-full shrink-0 sm:w-[300px]"><UniversePresence mood={step >= 7 ? "success" : step >= 6 ? "recommend" : step >= 4 ? "focus" : "calm"} sector={sector?.id} step={step} /></div><div><div className="flex items-center gap-2 label-mono text-[12px] uppercase tracking-[0.2em] text-primary"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" /> XR INTELLIGENCE</div><div className="truncate text-[14px] text-muted-foreground sm:text-xs">Je construis votre recommandation étape par étape.</div></div></div><div className="flex items-center gap-1.5"><div className="h-1 w-10 overflow-hidden rounded-full bg-muted sm:w-16"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${progress}%` }} /></div><span className="label-mono text-[12px] text-muted-foreground">{String(step + 1).padStart(2, "0")}/08</span></div></div>
    <div className="relative overflow-hidden rounded-[1.5rem] border border-border/80 bg-card/70 p-3 shadow-[0_30px_80px_-50px_rgba(0,0,0,.9)] backdrop-blur-2xl sm:p-5"><div className="pointer-events-none absolute -right-20 -top-20 h-48 w-48 rounded-full bg-primary/10 blur-3xl" /><div className="mb-4 flex items-end justify-between gap-2 border-b border-border/70 pb-3"><div><p className="label-mono text-[12px] uppercase tracking-[0.18em] text-muted-foreground">{step >= 7 ? "Finalisation" : `Question ${step + 1} sur 8`}</p><h2 className="mt-1 text-[clamp(1.4rem,3vw,2rem)] font-medium tracking-[-0.03em] sm:text-2xl">{titles[step]}</h2></div>{step > 0 && step < 7 && <button type="button" onClick={() => setStep(step - 1)} className="rounded-full border border-border px-2.5 py-1 text-[14px] text-muted-foreground hover:text-foreground">←</button>}</div>
      {step === 0 && <div><div className="mb-4 rounded-2xl border border-primary/15 bg-gradient-to-br from-primary/[.08] via-background to-transparent p-5"><div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[.14em]"><Globe2 className="h-4 w-4 text-primary"/>Votre univers</div><h3 className="mt-2 text-[clamp(1.4rem,3vw,2rem)] font-semibold">Dites-nous votre univers.</h3><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Choisissez votre famille de projet. La plaque XR Intelligence évolue immédiatement avec votre univers.</p></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">{(showAllSectors ? SECTORS : SECTORS.slice(0,6)).map((x,i) => <button key={x.id} type="button" onClick={() => { setSectorId(x.id); window.setTimeout(() => setStep(1), 180); }} className="group relative min-h-[116px] overflow-hidden rounded-2xl border border-border bg-background/70 p-3 text-left transition-all hover:-translate-y-0.5 hover:border-primary/45 hover:bg-primary/[.045]"><span className="absolute right-2 top-2 text-[12px] text-muted-foreground/50">0{String(i+1)}</span><span className="block pr-5 text-[14px] font-semibold leading-4 group-hover:text-primary">{x.label}</span><span className="mt-1.5 block text-[12px] leading-3.5 text-muted-foreground line-clamp-2">{x.detail}</span><span className="absolute bottom-2.5 right-3 h-1.5 w-1.5 rounded-full bg-primary/30 transition group-hover:scale-150 group-hover:bg-primary"/></button>)}</div><button type="button" onClick={() => setShowAllSectors(v=>!v)} className="mt-2 w-full rounded-xl border border-dashed border-border py-2.5 text-[14px] uppercase tracking-[.14em] text-muted-foreground transition hover:border-primary/35 hover:text-primary">{showAllSectors ? "Réduire les activités" : "Voir toutes les activités · Julie affine ensuite avec vous"}</button></div>}
      ...
      {step === 4 && <div><div className="mb-3 rounded-xl border border-primary/15 bg-primary/[0.035] p-3"><div className="text-[14px] font-medium text-primary">XR Intelligence a déjà fait le premier tri</div><p className="mt-1 text-[14px] leading-4 text-muted-foreground">Voici les leviers qui ont du sens pour votre activité. Ajustez-les librement avant de valider.</p></div><div className="relative mb-3 overflow-hidden rounded-[1.25rem] border border-primary/20 bg-gradient-to-br from-primary/[0.10] via-primary/[0.025] to-transparent p-3 shadow-sm sm:p-4"><div className="pointer-events-none absolute -right-8 -top-8 h-24 w-24 rounded-full bg-primary/15 blur-2xl" /><div className="flex items-center gap-2 text-[14px] font-semibold uppercase tracking-wider text-primary"><Sparkles className="h-3.5 w-3.5" /> Sélection préparée par XR Intelligence <span className="ml-1 rounded-full border border-primary/20 px-1.5 py-0.5 text-[12px]">profil + budget + objectif</span></div><p className="mt-2 text-[14px] leading-4 text-muted-foreground">XR Intelligence croise votre activité, votre objectif, votre situation, votre budget et votre acquisition pour éviter les prestations inutiles.</p></div><div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">{orderedServices.map((x) => card(x, () => toggleService(x.id), selectedServices.includes(x.id)))}</div><button type="button" disabled={!selectedServices.length} onClick={() => setStep(5)} className="mt-3 w-full rounded-full bg-primary px-5 py-3 text-[14px] font-semibold uppercase tracking-wider text-primary-foreground disabled:opacity-40">Valider ma sélection →</button></div>}
      {step === 5 && <div><p className="mb-2 text-[14px] text-muted-foreground">Dernière question d'acquisition.</p><div className="grid grid-cols-2 gap-1.5 sm:grid-cols-4">{DISCOVERY.map((x) => card(x, () => choose(setDiscovery, x.id, 6)))}</div></div>}
      {step === 6 && sector && <div className="space-y-3"><div className="relative overflow-hidden rounded-[1.5rem] border border-primary/30 bg-gradient-to-br from-primary/[0.10] to-transparent p-4 shadow-sm"><div className="pointer-events-none absolute -right-8 -top-8 h-24 w-24 rounded-full bg-primary/10 blur-2xl" /><div className="flex items-center gap-2 text-xs font-semibold"><Sparkles className="h-4 w-4 text-primary" /> Le conseil de XR Intelligence</div><p className="mt-2 text-xs leading-5 sm:text-sm">{recommendation}</p></div><div className="mb-2 text-[14px] uppercase tracking-[0.16em] text-muted-foreground">Le parcours que Julie vous conseille</div><div className="grid gap-2 sm:grid-cols-2">{selectedServices.map((id) => { const p = proposals.find((x) => x.id === id); return p ? <div key={id} className="flex items-center justify-between rounded-xl border border-border bg-background p-3"><div><div className="text-xs font-medium">{p.label}</div><div className="text-[14px] text-muted-foreground">{p.detail}</div></div><div className="text-right text-xs font-semibold text-primary">À partir de {p.price.toLocaleString("fr-FR")} €{p.period === "month" ? " / mois" : ""}</div></div> : null; })}</div><div className="rounded-2xl border border-primary/15 bg-primary/[0.035] p-4 text-xs shadow-sm"><div className="flex justify-between"><span>Prestations ponctuelles</span><strong>{once.toLocaleString("fr-FR")} €</strong></div><div className="mt-1 flex justify-between"><span>Abonnements mensuels</span><strong>{monthly.toLocaleString("fr-FR")} € / mois</strong></div></div><div className="grid gap-2 sm:grid-cols-2"><button type="button" onClick={() => setStep(7)} className="w-full rounded-full bg-primary px-5 py-3.5 text-[14px] font-semibold uppercase tracking-wider text-primary-foreground">Recevoir mon devis PDF →</button><a href={`https://wa.me/33767566783?text=${encodeURIComponent(`Bonjour XRAGENCY, j'ai terminé mon diagnostic XR Intelligence. Secteur : ${sector.label}. Budget : ${budgetChoice?.label}. Prestations recommandées : ${selectedServices.map((id) => proposals.find((p) => p.id === id)?.label).filter(Boolean).join(", ")}. Total : ${once.toLocaleString("fr-FR")} € + ${monthly.toLocaleString("fr-FR")} €/mois.`)}`} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full border border-border px-5 py-3.5 text-[14px] font-semibold uppercase tracking-wider"><MessageCircle className="h-3.5 w-3.5" /> Continuer sur WhatsApp</a></div></div>}
      {step === 7 && <div className="space-y-2.5"><p className="text-[14px] text-muted-foreground">Entrez vos coordonnées. XR Intelligence prépare le PDF avec le contexte, les prestations sélectionnées et les montants.</p><input value={client.company} onChange={(e) => setClient({ ...client, company: e.target.value })} placeholder="Nom de la société *" className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary" /><input value={client.name} onChange={(e) => setClient({ ...client, name: e.target.value })} placeholder="Votre nom *" className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary" /><input value={client.email} onChange={(e) => setClient({ ...client, email: e.target.value })} type="email" placeholder="E-mail *" className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary" /><input value={client.whatsapp} onChange={(e) => setClient({ ...client, whatsapp: e.target.value })} placeholder="WhatsApp / téléphone *" className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary" /> <input value={client.website} onChange={(e) => setClient({ ...client, website: e.target.value })} placeholder="Site internet (si vous en avez un)" className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary" /><button type="button" disabled={!client.company || !client.name || !client.email || !client.whatsapp || sending} onClick={generatePdf} className="w-full rounded-full bg-primary px-5 py-3.5 text-[14px] font-semibold uppercase tracking-wider text-primary-foreground disabled:opacity-40">{sending ? "Génération et envoi…" : generated ? "Générer / renvoyer le devis PDF →" : "Générer mon devis PDF →"}</button>{sendMessage && <div className="rounded-xl border border-primary/25 bg-primary/[0.05] p-3 text-center text-[14px] leading-5 text-muted-foreground">{sendMessage}</div>}<div className="grid grid-cols-2 gap-2"><a href={`https://wa.me/33767566783?text=${encodeURIComponent(`Bonjour XR Agency, je viens de préparer un devis avec XR Intelligence. Nom : ${client.name}. Email : ${client.email}. WhatsApp : ${client.whatsapp}. Total estimé : ${once.toLocaleString("fr-FR")} € + ${monthly.toLocaleString("fr-FR")} €/mois.`)}`} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl border border-border px-3 py-3 text-[14px] font-semibold uppercase tracking-wider"><MessageCircle className="h-3.5 w-3.5" /> WhatsApp</a><a href={`mailto:${client.email}?subject=${encodeURIComponent("Votre devis XRAGENCY")}&body=${encodeURIComponent("Votre devis PDF a été généré avec XR Intelligence. Si l'envoi automatique n'est pas disponible, vous pouvez partager le PDF généré depuis votre appareil.")}`} className="inline-flex items-center justify-center gap-2 rounded-xl border border-border px-3 py-3 text-[14px] font-semibold uppercase tracking-wider"><Mail className="h-3.5 w-3.5" /> E-mail</a></div><button type="button" onClick={reset} className="mx-auto flex items-center gap-1 text-[14px] text-muted-foreground hover:text-foreground"><RotateCcw className="h-3 w-3" /> Recommencer</button></div>}
    </div></div></section>;
}
