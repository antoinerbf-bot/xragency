...
export function QuoteConfiguratorCompact() {
  ...
  return <section id="quote" className="relative border-y border-border/50 bg-background py-4 sm:py-6"><div className="mx-auto max-w-2xl px-3.5 sm:px-5"><div className="mb-3 flex items-center justify-between gap-2"><div className="flex min-w-0 items-center gap-2.5"><div className="shrink-0"><AlexandreAdvisor /></div><div><div className="flex items-center gap-2 label-mono text-[8px] uppercase tracking-[0.2em] text-primary"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" /> XR INTELLIGENCE · ALEXANDRE</div><div className="truncate text-[9px] text-muted-foreground sm:text-xs">Je construis votre recommandation étape par étape.</div></div></div><div className="flex items-center gap-1.5"><div className="h-1 w-10 overflow-hidden rounded-full bg-muted sm:w-16"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${progress}%` }} /></div><span className="label-mono text-[7px] text-muted-foreground">{String(step + 1).padStart(2, "0")}/08</span></div></div>
    <div className="relative overflow-hidden rounded-[1.5rem] border border-border/80 bg-card/70 p-3 shadow-[0_30px_80px_-50px_rgba(0,0,0,.9)] backdrop-blur-2xl sm:p-5"><div className="pointer-events-none absolute -right-20 -top-20 h-48 w-48 rounded-full bg-primary/10 blur-3xl" /><div className="mb-4 flex items-end justify-between gap-2 border-b border-border/70 pb-3"><div><p className="label-mono text-[7px] uppercase tracking-[0.18em] text-muted-foreground">{step >= 7 ? "Finalisation" : `Question ${step + 1} sur 8`}</p><h2 className="mt-1 text-base font-medium tracking-[-0.03em] sm:text-2xl">{titles[step]}</h2></div>{step > 0 && step < 7 && <button type="button" onClick={() => setStep(step - 1)} className="rounded-full border border-border px-2.5 py-1 text-[10px] text-muted-foreground hover:text-foreground">←</button>}</div>
      {step === 0 && <div><div className="mb-3 rounded-xl border border-primary/15 bg-primary/[0.035] p-3"><div className="flex items-center gap-2 text-[10px] font-medium"><Globe2 className="h-3.5 w-3.5 text-primary"/>Choisissez votre activité</div><p className="mt-1 text-[9px] text-muted-foreground">Ouvrez le menu pour parcourir tous les secteurs. Pas besoin de trouver une catégorie parfaite.</p></div><div className="relative"><select value={sectorId} onChange={(e) => { setSectorId(e.target.value); if(e.target.value) window.setTimeout(() => setStep(1), 160); }} className="w-full appearance-none rounded-2xl border border-primary/25 bg-background px-4 py-4 pr-12 text-sm font-medium outline-none focus:border-primary"><option value="">Sélectionner votre activité…</option>{SECTORS.map(x => <option key={x.id} value={x.id}>{x.label} — {x.detail}</option>)}</select><ChevronDown className="pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-primary"/></div><div className="mt-3 grid grid-cols-2 gap-1.5 sm:grid-cols-3">{SECTORS.slice(0,6).map(x => card(x, () => choose(setSectorId, x.id, 1), sectorId === x.id))}</div></div>}
