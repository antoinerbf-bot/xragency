...
export function QuoteConfiguratorCompact() {\n  useEffect(() => { const params = new URLSearchParams(window.location.search); const requested = params.get("service"); if (!requested) return; const map: Record<string,string> = { websites:"website", ecommerce:"website", maintenance:"maintenance", webcare:"maintenance", social:"social", branding:"branding", seo:"seo", maps:"maps", ads:"ads" }; const id = map[requested] ?? requested; if (SERVICES.some((item) => item.id === id)) { setSelectedServices([id]); setStep(4); window.setTimeout(() => document.getElementById("quote")?.scrollIntoView({ behavior: "smooth" }), 80); } }, []);
  ...
  const titles = ["Votre activité", "Votre priorité", "Votre situation aujourd'hui", "L'investissement envisagé", "Les prestations à activer", "Comment vos clients vous trouvent", "Votre recommandation", "Vos informations"]; const progress = ((step + 1) / 8) * 100; const card = (item: Choice, onClick: () => void, selected = false) => <button key={item.id} type="button" onClick={onClick} className={`group relative w-full overflow-hidden rounded-[1.15rem] border p-3.5 text-left transition-all duration-300 hover:-translate-y-1 hover:border-primary/55 hover:shadow-[0_22px_45px_-25px_rgba(0,0,0,.85)] ${selected ? "border-primary bg-primary/[0.09] shadow-[0_18px_45px_-25px_hsl(var(--primary)/.45)]" : "border-border/80 bg-background/70"}`}><span className="pointer-events-none absolute -right-5 -top-5 h-16 w-16 rounded-full bg-primary/[0.05] blur-xl transition-transform duration-500 group-hover:scale-150" /><div className="relative flex items-start justify-between gap-3"><div><span className="mb-2 inline-flex rounded-full border border-border/70 bg-background/70 px-2 py-1 label-mono text-[12px] uppercase tracking-[.14em] text-muted-foreground">XR Intelligence · {selected ? "retenu" : "option"}</span><span className="block text-[15px] font-medium leading-tight sm:text-sm">{item.label}</span><span className="mt-1.5 block text-[14px] leading-4 text-muted-foreground">{item.detail}</span></div><span className={`mt-1 grid h-6 w-6 shrink-0 place-items-center rounded-full border transition-all ${selected ? "border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground group-hover:border-primary/50 group-hover:text-primary"}`}>{selected ? <Check className="h-3 w-3" /> : <ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-0.5" />}</span></div></button>;

  return <section id="quote" className="relative border-y border-border/50 bg-background py-4 sm:py-6"><div className="mx-auto max-w-6xl px-3.5 sm:px-6 lg:px-8"><div className="mb-4 flex items-center justify-between gap-3">
      <div><div className="flex items-center gap-2 text-sm uppercase tracking-[.2em] text-primary"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" /> XR INTELLIGENCE</div><div className="mt-1 text-sm text-muted-foreground">Je construis votre recommandation étape par étape.</div></div>
      <div className="flex items-center gap-2"><div className="h-1 w-12 overflow-hidden rounded-full bg-muted sm:w-20"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${progress}%` }} /></div><span className="font-mono text-sm text-muted-foreground">{String(step + 1).padStart(2, "0")}/08</span></div>
    </div>
    <div className="relative overflow-hidden rounded-[1.5rem] border border-border/80 bg-card/70 p-3 shadow-[0_30px_80px_-50px_rgba(0,0,0,.9)] backdrop-blur-2xl sm:p-5"><div className="pointer-events-none absolute -right-20 -top-20 h-48 w-48 rounded-full bg-primary/10 blur-3xl" /><div className="mb-4 flex items-end justify-between gap-2 border-b border-border/70 pb-3"><div><p className="label-mono text-[12px] uppercase tracking-[0.18em] text-muted-foreground">{step >= 7 ? "Finalisation" : `Question ${step + 1} sur 8`}</p><h2 className="mt-1 text-[clamp(1.4rem,3vw,2rem)] font-medium tracking-[-0.03em] sm:text-2xl">{titles[step]}</h2></div>{step > 0 && step < 7 && <button type="button" onClick={() => setStep(step - 1)} className="rounded-full border border-border px-2.5 py-1 text-[14px] text-muted-foreground hover:text-foreground">←</button>}</div>
      {step === 0 && <div className="grid gap-5 lg:grid-cols-[minmax(300px,380px)_1fr] lg:items-start">
        <UniversePresence mood="calm" sector={sector?.id} step={step} />
        <div>
          <div className="mb-4 rounded-2xl border border-primary/15 bg-gradient-to-br from-primary/[.08] via-background to-transparent p-5">
            <div className="text-sm font-semibold uppercase tracking-[.14em] text-primary">Votre univers</div>
          ...
          <button type="button" onClick={() => setShowAllSectors(v => !v)} className="mt-3 min-h-[44px] w-full rounded-xl border border-border py-3 text-sm uppercase tracking-[.12em] text-muted-foreground transition hover:border-primary/45 hover:text-primary">{showAllSectors ? "Réduire les activités" : "Voir toutes les activités"}</button>
        </div>
      </div>}
