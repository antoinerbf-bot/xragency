...
export function QuoteConfiguratorCompact() {
  ...
  return <section id="quote" className="relative border-y border-border/50 bg-background py-4 sm:py-6"><div className="mx-auto max-w-[1320px] px-4 sm:px-6"><div className="grid gap-5 lg:grid-cols-[320px_minmax(0,1fr)] lg:items-start"><UniversePresence sector={sector?.id} mood={step >= 7 ? "success" : step >= 6 ? "recommend" : step >= 4 ? "focus" : "calm"} /><div><div className="mb-3 flex items-center justify-between gap-2"><div className="flex min-w-0 items-center gap-2.5"><div><div><div className="flex items-center gap-2 label-mono text-xs uppercase tracking-[0.2em] text-primary"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" /> XR INTELLIGENCE · DEVIS</div><div className="truncate text-sm text-muted-foreground sm:text-xs">Votre recommandation se construit en 45 secondes.</div></div></div><div className="flex items-center gap-1.5"><div className="h-1 w-10 overflow-hidden rounded-full bg-muted sm:w-16"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${progress}%` }} /></div><span className="label-mono text-xs text-muted-foreground">{String(step + 1).padStart(2, "0")}/08</span></div></div>
    <div className="relative overflow-hidden rounded-[1.5rem] border border-border/80 bg-card/70 p-4 shadow-[0_30px_80px_-50px_rgba(0,0,0,.9)] backdrop-blur-2xl sm:p-5"><div className="pointer-events-none absolute -right-20 -top-20 h-48 w-48 rounded-full bg-primary/10 blur-3xl" /><div className="mb-4 flex items-end justify-between gap-2 border-b border-border/70 pb-3"><div><p className="label-mono text-xs uppercase tracking-[0.18em] text-muted-foreground">{step >= 7 ? "Finalisation" : `Question ${step + 1} sur 8`}</p><h2 className="mt-1 text-base font-medium tracking-[-0.03em] sm:text-2xl">{titles[step]}</h2></div>{step > 0 && step < 7 && <button type="button" onClick={() => setStep(step - 1)} className="rounded-full border border-border px-2.5 py-1 text-sm text-muted-foreground hover:text-foreground">←</button>}</div>
      {step === 0 && <div><div className="mb-4 rounded-2xl border border-primary/15 bg-gradient-to-br from-primary/[.08] via-background to-transparent p-4"><div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[.14em]"><Globe2 className="h-3.5 w-3.5 text-primary"/>Votre univers</div><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Choisissez votre activité. XR Intelligence adapte ensuite la recommandation à votre objectif, votre situation et votre budget.</p></div><div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">{(showAllSectors ? SECTORS : SECTORS.slice(0,6)).map((x,i) => <button key={x.id} type="button" onClick={() => { setSectorId(x.id); window.setTimeout(() => setStep(1), 180); }} className="group relative min-h-[116px] touch-manipulation overflow-hidden rounded-2xl border border-border bg-background/70 p-4 text-left transition-all hover:-translate-y-1 hover:border-primary/45 hover:bg-primary/[.045] focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary"><span className="absolute right-3 top-3 font-mono text-xs text-primary/70">0{String(i+1)}</span><span className="block pr-5 text-[15px] font-semibold leading-5 group-hover:text-primary">{x.label}</span><span className="mt-1.5 block text-[13px] leading-5 text-muted-foreground line-clamp-2">{x.detail}</span><span className="absolute bottom-2.5 right-3 h-1.5 w-1.5 rounded-full bg-primary/30 transition group-hover:scale-150 group-hover:bg-primary"/></button>)}</div><button type="button" onClick={() => setShowAllSectors(v=>!v)} className="mt-2 w-full rounded-xl border border-dashed border-border py-2.5 text-sm uppercase tracking-[.14em] text-muted-foreground transition hover:border-primary/35 hover:text-primary">{showAllSectors ? "Réduire les activités" : "Voir toutes les activités"}</button></div>}
