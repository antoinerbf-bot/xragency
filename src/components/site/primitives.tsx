...
/** Scroll-triggered reveal with 2026 kinetic easing */
export function Reveal({
  ...
  const animationName = variant === "kinetic" ? "kinetic-in" : "ember-rise";
  const duration = variant === "kinetic" ? "0.9s" : "0.8s";
  ...
  return (
    <div
      ...
      style={
        shown
          ? {
              animation: `${animationName} ${duration} cubic-bezier(0.16, 1, 0.3, 1) ${delay}ms both`,
            }
          : { opacity: 0 }
      }
    >
...
export function SectionHeading({
  ...
  return (
    <Parallax speed={-0.03} className="max-w-3xl">
      ...
      <Reveal delay={80} variant="kinetic">
...
export function EmberButton({
  ...
  const base =
    "label-mono inline-flex items-center justify-center gap-2 rounded-full px-7 py-3.5 min-h-[44px] transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-40 cursor-pointer btn-lift";
  const styles = {
    solid:
