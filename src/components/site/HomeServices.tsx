...
const DISPLAY_PRICE: Record<string, number> = { websites: 499, branding: 199, seo: 299, maps: 999, social: 499, maintenance: 29, ecommerce: 1499 };
...
export function HomeServices() {
  ...
  useEffect(() => {
    const section = document.getElementById("homepage-services"); if (!section) return;
    let frame = 0;
    const update = () => { frame = 0; const rect = section.getBoundingClientRect(); const travel = Math.max(section.offsetHeight - window.innerHeight, 1); setProgress(Math.max(0, Math.min(1, -rect.top / travel))); };
    ...
    <section id="homepage-services" aria-label="Les sept expertises XR Agency" className="relative bg-background" style={{ height: `${Math.max(featured.length, 1) * 100}svh` }}>
      <div className="sticky top-0 h-[100svh] min-h-[680px] overflow-hidden">
        ...
        <div className="absolute left-5 right-5 top-24 z-30 flex items-start justify-between sm:left-8 sm:right-8 lg:left-12 lg:right-12 lg:top-28">
          <div><span className="label-mono text-[9px] uppercase tracking-[0.3em] text-primary">XR AGENCY · 07 DISCIPLINES</span><h2 className="display-serif mt-3 max-w-3xl text-4xl leading-[0.88] sm:text-6xl lg:text-7xl">Une expertise.<br />Puis la suivante.</h2></div>
        ...
        <div className="absolute inset-x-0 bottom-0 top-0 mx-auto max-w-[1600px]">
          {featured.map((service, index) => {
            const distance = index - position; const abs = Math.abs(distance); const active = abs < 0.55;
            const scale = 1 - Math.min(abs * 0.07, 0.22); const opacity = abs > 1.45 ? 0 : Math.max(0.12, 1 - abs * 0.58);
            return <article key={service.id} className="absolute inset-x-5 bottom-10 top-52 origin-center transition-[opacity,transform] duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] sm:inset-x-8 sm:bottom-12 sm:top-48 lg:inset-x-12 lg:bottom-14 lg:top-44" style={{ zIndex: 20 - Math.round(abs * 2), opacity, transform: `translate3d(${distance * 11}%, ${distance * 4}%, 0) scale(${scale})`, pointerEvents: active ? "auto" : "none" }}>
              <div className="relative h-full overflow-hidden rounded-[2rem] border border-white/10 bg-black shadow-2xl sm:rounded-[2.5rem]">
                <img src={SERVICE_IMG[service.id]} alt="" aria-hidden className="absolute inset-[-5%] h-[110%] w-[110%] object-cover transition-transform duration-[1400ms]" style={{ transform: `scale(${active ? 1.04 : 1.1}) translate3d(${distance * -1.5}%, ${distance * -1}%, 0)` }} />
                ...
                <div className="relative z-10 flex h-full flex-col justify-end p-7 sm:p-10 lg:p-16"><div className="max-w-5xl">
                  <div className="mb-5 flex items-center gap-3 label-mono text-[9px] uppercase tracking-[0.24em] text-white/45"><span className="text-primary">{DISPLAY_NUM[service.id]}</span><span className="h-px w-10 bg-white/20" /><span>XR AGENCY</span></div>
                  ...
                  <div className="mt-7 flex flex-wrap items-center gap-3"><Link to="/services/$serviceId" params={{ serviceId: service.id }} className="group inline-flex items-center gap-3 rounded-full bg-white px-5 py-3 label-mono text-[9px] font-semibold uppercase tracking-[0.12em] text-black transition-all duration-300 hover:gap-5">Découvrir la prestation <ArrowUpRight className="h-4 w-4" /></Link><span className="rounded-full border border-white/15 bg-white/5 px-4 py-3 label-mono text-[9px] uppercase tracking-[0.12em] text-white/60 backdrop-blur-xl">À partir de {price(DISPLAY_PRICE[service.id])}{service.fromPeriod === "month" ? " / mois" : service.fromPeriod === "year" ? " / an" : ""}</span></div>
                </div><div className="mt-8 flex items-center justify-between border-t border-white/15 pt-4"><div className="flex gap-1.5">{featured.map((item, dotIndex) => <span key={item.id} className={cn("h-1 rounded-full transition-all duration-500", dotIndex === index ? "w-10 bg-white" : "w-2 bg-white/25")} />)}</div><div className="flex items-center gap-2 label-mono text-[9px] uppercase tracking-[0.16em] text-white/40">{index < featured.length - 1 ? <>Scroller <ArrowDown className="h-3 w-3 animate-bounce" /></> : "Explorer l'expertise"}</div></div></div>
              </div>
            </article>;
          })}
        </div>
