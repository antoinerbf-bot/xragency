...
function commandExists(cmd) {
  ...
  try {
    if (isWindows) {
      ...
      return (
        spawnSync("sh", ["-c", `command -v "$1"`, "--", cmd], { stdio: "ignore" }).status === 0
      );
    }
  } catch {
    return false;
...
function main() {
  ...
  if (checkVercel()) {
    ...
    console.log(
      JSON.stringify({ status: "already_installed", message: "Vercel CLI already installed" }),
    );
    process.exit(0);
  }
  ...
  if (checkVercel()) {
    ...
    console.log(
      JSON.stringify({ status: "success", message: "Vercel CLI installed successfully" }),
    );
  } else {
